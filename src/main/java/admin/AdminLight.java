 ...
 */
@WebServlet("/AdminLight")
public class AdminLight extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		out.append("alert('Đã xóa sản phẩm thành công!');"); // Sử dụng alert để hiển thị thông báo
		out.append("}");
		out.append("});");
		out.append("</script>");
		...
		out.append("<div class=\"row d-flex align-items-center\">");
		out.append("<div class=\"col\">");
		out.append("<h1>Danh sách sản phẩm</h1>");
		out.append("<nav>");
		out.append("<ol class=\"breadcrumb\">");
		out.append("<li class=\"breadcrumb-item\"><a href=\"View\">Trang chủ</a></li>");
		out.append("<li class=\"breadcrumb-item\">Sản Phẩm</li>");
		out.append("<li class=\"breadcrumb-item active\">Danh sách sản phẩm</li>");
		out.append("</ol>");
		out.append("</nav>");
		out.append("</div>");
		out.append("<div class=\"col-lg-3\">");
		out.append(
				"<a href=\"/GocDecore/AddLightView\"><button class=\"btn border border-danger\"><i class=\"bi bi-file-plus-fill\"></i> Thêm mới sản phẩm</button></a>");
		out.append("</div>");
		out.append("</div>");
		...
		out.append("<h5 class=\"card-title\" style=\"text-align: center;\"><strong>DANH SÁCH SẢN PHẨM</strong></h5>");
		out.append("");

		out.append("<!-- Table with stripped rows -->");
		out.append("<table class=\"table datatable table-striped light_img\">");
		out.append("<thead>");
		out.append("<tr>");
		out.append("<th class=\"tableID\">ID</th>");
		out.append("<th>Tên đèn</th>");
		out.append("<th>Giá</th>");
		out.append("<th>Giá gốc</th>");
		out.append("<th>Giảm giá %</th>");
		out.append("<th>Chất liệu</th>");
		out.append("<th>Màu sắc</th>");
		out.append("<th>Kích thước</th>");
		out.append("<th>Tình trạng</th>");
		out.append("<th>Hình ảnh</th>");
		out.append("<th>Phân loại</th>");
		out.append("<th>Điểm đánh giá</th>");
		out.append("<th>Lượt bán</th>");
		out.append("<th>Sửa</th>");
		...
		ArrayList<LightObject> list = lf.getLights(); // Gọi phương thức getLights để lấy dữ liệu

		// Gán Danh sách sản phẩm vào thuộc tính request
		request.setAttribute("light", list);

		// Lấy Danh sách sản phẩm từ thuộc tính request
		ArrayList<LightObject> lightList = (ArrayList<LightObject>) request.getAttribute("light");
		...
		if (lightList != null && !lightList.isEmpty()) {
			for (LightObject item : list) {
				...
				out.append("<td>" + decimalFormat.format(item.getLight_price()) + "</td>");
				out.append("<td>" + decimalFormat.format(item.getLight_pricelegal()) + "</td>");
				out.append("<td>" + item.getLight_percen() + "</td>");
				out.append("<td>" + item.getLight_material() + "</td>");
				out.append("<td>" + item.getLight_color() + "</td>");
				out.append("<td>" + item.getLight_size() + "</td>");
				out.append("<td>" + item.getLight_state() + "</td>");
				out.append("<td><img src=\"img/light_img/"+item.getLight_img()+".jpg\" alt=\"\"></td>");
				out.append("<td>" + item.getCategory_id() + "</td>");
				out.append("<td>" + item.getRating() + "</td>");
				out.append("<td>" + item.getSales_count() + "</td>");
				out.append("<td><a href=\"/GocDecore/edit?id=" + item.getLight_id()
						+ "\"><i class=\"bi bi-pencil-square\"></i></a></td>");
				out.append("<td><a href=\"/GocDecore/DelLightView?id=" + item.getLight_id()
						+ "\" onclick='return confirm(\"Bạn có chắc chắn muốn xóa sản phẩm này?\")'><i class='bi bi-trash'></i></a></td>");
				out.append("</tr>");
			}
		} else {
